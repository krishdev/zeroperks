 ...
 * @param {Object} eventDetails | eventName, eventBanner.url, reminderEmailContent, url
 * 
 */
exports.reminderEmailEvt = async (email, eventDetails) => {
  try {
    const templatePath = path.resolve(__dirname, '../views/email/event-reminder.ejs');
      ejs.renderFile(templatePath, { 
          title: eventDetails.eventName,
          invitationBanner: !eventDetails.eventBanner.custom ? `https://admin.zeroperks.com/${eventDetails.eventBanner.url}` : eventDetails.eventBanner.url,
          content: eventDetails.reminderEmailContent,
          inviteLink: !eventDetails.eventBanner.custom ? `https://zeroperks.com/event/${eventDetails.url}` : eventDetails.url
      }, (err, htmlFile) => {
          if (err) {
    ...
    await sendEmail({
      from: 'emailzeroperks@gmail.com',
      to: 'mailkrishna2@gmail.com',
      subj: 'Error: Reminder email 144!',
      content: `Email error: ${email} ${error}`
    });
    return true;
  }
}